...
* The ``compare_properties`` module searches for matches for the protein sequences in the dataset using the Smith-Waterman local alignment algorithm.
...
* The ``model_trainer`` module sets up the training scheme and model structure. 
* The ``evalute_models`` module conducts the main experiments, which compare the SpinML and SeqSpinML models across metrics, generate comparative plots, show SHAP importance values across spinning conditions, and conduct an ablation study. 
